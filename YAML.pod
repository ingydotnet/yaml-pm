...
NOTE: It is currently illegal to not begin a YAML stream with a
      separator if there are more than one documents in the YAML stream.
...
Setting the PerlCode option is a shortcut to set both the StoreCode and
LoadCode options at once. Setting PerlCode to '1' tells YAML.pm to dump
...
C<ysh> has a few handy command line options:

=over 4

=item -r

Test round-tripping. When you enter Perl code, ysh will Store it, Load
it and Store it again. If the two Stores don't match, it will bark.

=item -R

Same as -r except that it also prints a confirmation when the data
B<does> round-trip.

=item -c

Sets $YAML::PerlCode to '1' automatically. This allows you to dump Perl
code references.

=item -l

Turns on logging. Will write the log to './ysh.log'. Concatenates to the
log file if the file already exists.
...
Same as -v, but prints the version of perl and YAML related
modules as well.
...
(ysh) with logging turned on. When you have successfully reproduced the
